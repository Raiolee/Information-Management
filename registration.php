...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  $path = mysqli_real_escape_string($connection, $_POST["image"]);
  $status = "Active";

  if (count($errors) == 0) {
    // Insert user data into the database
    $insertQuery = "INSERT INTO users (user_email, user_password, user_fname, user_lname, user_mobile, user_gender, user_dob, user_address, user_type, user_path, user_status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    // $hashedPassword = password_hash($password, PASSWORD_DEFAULT); // Commented out hashing function
    $stmt = $connection->prepare($insertQuery);
    $stmt->bind_param('sssssssssss', $email, $password, $fname, $lname, $mobilenumber, $gender, $bday, $address, $type, $path, $status);
...
<body>
  ...
  <div class="container">
    <div class="d-flex justify-content-center align-items-center my-4">
      <form id="registration" class="w-25" method="POST">
        ...
        <div class="mb-3">
          ...
          <input type="date" name="dob" class="form-control datepicker" id="datepicker">
        </div>
        <div class="mb-3">
    ...
    <div class="text-center mt-3">
      <p>Already have an account? <a href="registration.php">Login here</a>.</p>
    </div>
  </div>
